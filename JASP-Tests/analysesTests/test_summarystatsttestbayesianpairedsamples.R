...
test_that("T-Test Paired Samples Init", {
    output <- SummaryStatsTTestBayesianPairedSamples(
                options = options, 
                perform = perform, 
                callback = NULL
            )
...
test_that("T-Test Paired Samples Run", {
    ...
    expect_equal(output$state$rowsTTestBayesianPairedSamples, expectedOutput)
})
