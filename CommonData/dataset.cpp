...
void DataSet::dbCreate()
{
	...
	_dataSetID	= db().dataSetInsert(_dataFilePath, _dataFileTimestamp, _description, _databaseJson, _emptyValues->toJson().toStyledString(), _dataFileSynch, _showRSyntax, _csvDelimiter, _importLocale);
...
{
	assert(_dataSetID > 0);
	db().dataSetUpdate(_dataSetID, _dataFilePath, _dataFileTimestamp, _description, _databaseJson, _emptyValues->toJson().toStyledString(), _dataFileSynch, _showRSyntax, _csvDelimiter, _importLocale);
	incRevision();
}
...
void DataSet::dbLoad(int index, std::function<void(float)> progressCallback, Version doUpgradeFrom)
{
	...
	db().dataSetLoad(_dataSetID, _dataFilePath, _dataFileTimestamp, _description, _databaseJson, emptyVals, _revision, _dataFileSynch, _showRSyntax, _csvDelimiter, _importLocale);
