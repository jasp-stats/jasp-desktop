...
bool Reporter::checkReports()
{
		...
		{
			somethingToReportFile.write(("Needed reports: " + std::to_string(_reportsNeeded)).c_str());
			somethingToReportFile.close();
		}
		else
		{
			std::cerr << "Reporter::checkReports: Could not open file for writing!" << std::endl;
		}

		return _reportsNeeded > 0;
}
...
void Reporter::writeReportLog()
{
		...
		if (reportLogFile.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
		{
			...
			reportLogFile.write((writeThis.join(',') + "\n").toUtf8());
			reportLogFile.close();
		}
		else
		{
			std::cerr << "Reporter::writeReportLog: Failed to open log file for appending!" << std::endl;
		}
}
...
void Reporter::onPdfPrintingFinishedHandler(QString pdfPath)
{
		...
		{
			reportComplete.write(QDateTime::currentDateTimeUtc().toString(Qt::ISODate).toUtf8());
			reportComplete.close();
		}
}
