...
void ODSXmlContentsHandler::processEndElement(const QXmlStreamReader &reader)
{
		...
		{
		case document_content:
				if (localName == _nameDocContent) _docDepth = not_in_doc;
				break;
		case body:
				if (localName == _nameBody) _docDepth = document_content;
				break;
		case spreadsheet:
				if (localName == _nameSpreadsheet) _docDepth = body;
				break;
		case table:
				if (localName == _nameTable) { _docDepth = spreadsheet; _tableRead = true; }
				break;
		case table_row:
				if (localName == _nameTableRow) {
					_docDepth = table;
		...
		case table_cell:
				if (localName == _nameTableCell) {
						if(_row == 0) {
								if (!_currentCell.isEmpty()) {
										...
										ods::ODSImportColumn & col = _dataSet->createColumn(_currentCell.toStdString());
										if(!_currentComment.isEmpty()) col.setTitle(fq(_currentComment));
								...
								if((!_currentCell.isEmpty() || !_currentComment.isEmpty()) && _column + _colRepeat != _excelMaxCols) {
										for (int i = 0; i < _colRepeat; i++) {
												ods::ODSImportColumn & col = _dataSet->getOrCreate(_column + i);
												col.setValue(_row - 1, fq(_currentCell));
												if(!_currentComment.isEmpty()) col.setComment(_row - 1, fq(_currentComment));
										}
				...
				break;
		case annotation:
				if (localName == _nameAnnotation) _docDepth = table_cell;
				break;
		case text_annotation:
				if (localName == _nameText) _docDepth = annotation;
				break;
		case text:
				if (localName == _nameText) _docDepth = table_cell;
				break;
		default: break;
		}
}
...
void ODSXmlContentsHandler::processCharacters(const QString &ch)
{
		...
		if (_docDepth == text) {
				if(_currentCell.isEmpty()) _currentCell.push_back(ch);
		} else if (_docDepth == text_annotation) {
				if(!_currentComment.isEmpty()) _currentComment.push_back("\t");
