...
public:
	...
	virtual			std::string			valueLookupAsShown(size_t row)			const	{ return valueLookup(row); } ///< What Column::getValue gives for valueLookup(row) once imported, which is what a sync compares
