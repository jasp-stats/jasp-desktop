...
void DataSetPackage::setSynchingExternally(bool synchingExternally_)
{
	...
	{
		if(_database != Json::nullValue)	databaseStartSynching(true);
		else if(_dataFilePath == "")		emit askUserForExternalDataFile();
		else								PreferencesModel::prefs()->setDataAutoSynchronization(true);
	}
