# loosely based on https://github.com/dewittpe/R-install-dependencies
...
# get the packages that JASP expects and put them in an env that includes a specific version
source("../../JASP-Engine/JASP/R/packagecheck.R", local=TRUE)
expected   <- .expectedPackages()
expEnv     <- new.env(hash = TRUE, parent = parent.frame(), size = length(expected))
pkgs       <- expected[,1]
...
# Now we must make sure that all pkgs are preceded by their dependencies and they by theirs
orderedPkgs   <- list()
...
while(i <= length(orderedPkgs))
{
  ...
  if(!exists(curPkg, where=expEnv, inherits=FALSE)) #if so get the version from available pkgs
  {
    ...
    print(paste0("Couldn't find version in expected packages so took it from availablePackages: ", expEnv[[curPkg]]))    
  }
...
system2("cat",args=c("org.jasp.JASP_header.json", jsonFile, "org.jasp.JASP_footer.json"), stdout="org.jasp.JASP.json")
