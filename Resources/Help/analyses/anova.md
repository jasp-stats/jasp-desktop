...
- Components and model terms 
    ...
    - Model terms: The independent variables being used in the model. By default all the independent variables included in the model are placed in this box, plus the interaction between these independent variables.  
- Sum of Squares 
...
- Factors: These are the independent variables included in the analysis. 
...
#### Post Hoc Tests: 
If the ANOVA is significant, you can follow up the analysis by performing a post hoc test to see which levels of a independent variable exactly differ. To perform a post hoc test drop the factor to perform the post hoc test on in the right column. Then you can select:    
- Effect size: By selecting this option the effect size, referred as the magnitude of the observed effect, will be presented. The used measure for the effect size is Cohen's d. The effect size will only be presented for the hoc type `Standard`. 
- Confidence intervals: When this option is selected, the confidence interval for the mean difference is calculated. This is done for every correction method except for Dunn. By default this is set to 95% but you can adjust this into the desired percentage.  
...
- Homogeneity Correction: This column is only available when this option is selected under `Assumption Checks`--> `Homogeneity corrections`. The corrections are only available for one-way ANOVA. 
    - None: No correction is applied. 
    - Brown-Forsythe: The Brown-Forsythe correction is used. 
    - Welch: The Welch correction is applied. 
...
##### Q-Q Plot: 
With Q-Q plot we inspect the normality of the residuals. The Theoretical Quantiles are presented on the x-axis and Standardized Residuals on y-axis. If the dots ly on the diagonal line, this indicates normality. 
...
- Estimate: The estimated difference between the compared levels. 
