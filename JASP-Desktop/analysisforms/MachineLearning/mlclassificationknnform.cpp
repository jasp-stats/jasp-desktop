...
#include "mlclassificationknnform.h"
#include "ui_mlclassificationknnform.h"

MLClassificationKNNForm::MLClassificationKNNForm(QWidget *parent) :
	AnalysisForm("MLClassificationKNNForm", parent),
	ui(new Ui::MLClassificationKNNForm)
{
	ui->setupUi(this);
    ...
    _targetListModel->setVariableTypesSuggested(Column::ColumnTypeNominal);
    _targetListModel->setVariableTypesAllowed(Column::ColumnTypeNominal | Column::ColumnTypeNominalText);
...
MLClassificationKNNForm::~MLClassificationKNNForm()
{
	delete ui;
}

void MLClassificationKNNForm::defaultOptions()
{
...
void MLClassificationKNNForm::bindTo(Options *options, DataSet *dataSet)
{
...
void MLClassificationKNNForm::factorsChanging()
{
	if (_options != NULL)
		_options->blockSignals(true);
}

void MLClassificationKNNForm::factorsChanged()
{
	if (_options != NULL)
		_options->blockSignals(false);
}

void MLClassificationKNNForm::on_manual_1_clicked(bool checked)
{
	if (checked) {
...
void MLClassificationKNNForm::on_auto_1_clicked(bool checked)
{
	if (checked) {
		ui->trainingDataManual->hide();
	}
...
void MLClassificationKNNForm::on_manual_2_clicked(bool checked)
{
	if (checked) {
...
void MLClassificationKNNForm::on_auto_2_clicked(bool checked)
{
	if (checked) {
		ui->nearestNeighboursCount->hide();
		ui->optimizedFrom->hide();
...
void MLClassificationKNNForm::on_optimized_2_clicked(bool checked)
{
	if (checked) {
		ui->nearestNeighboursCount->hide();
...
void MLClassificationKNNForm::on_manual_3_clicked(bool checked)
{
	if (checked) {
...
void MLClassificationKNNForm::on_auto_3_clicked(bool checked)
{
	if (checked) {
		ui->distanceParameterManual->hide();
	}
...
void MLClassificationKNNForm::on_optimized_3_clicked(bool checked)
{
	if (checked) {
		ui->distanceParameterManual->hide();
	}
