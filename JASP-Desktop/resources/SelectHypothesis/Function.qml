...
Item {
    ...
    property variant functionNameToImageSource: {
        "sum": "qrc:/icons/sum.png",
        "prod": "qrc:/icons/product.png",
        "sd": "qrc:/icons/sigma.png",
        "var": "qrc:/icons/variance.png",
        "!": "qrc:/icons/negative.png",
        "sqrt": "qrc:/icons/rootHead.png"
    }
    property string functionImageSource: functionNameToImageSource[functionName]
                                         !== undefined ? functionNameToImageSource[functionName] : ""
    property bool isNested: false
    property var booleanReturningFunctions: ["!"]

    property var dragKeys: booleanReturningFunctions.indexOf(
                               functionName) >= 0 ? ["boolean"] : ["number"]
    ...
    readonly property bool showParentheses: false // !isMean && (parameterNames.length > 1 || isAbs || functionImageSource === "")

    property real extraMeanWidth: (isMean ? 10 : 0)

    height: meanBar.height + Math.max(dropRow.height,
                                      filterConstructor.blockDim)
    width: functionDef.width + haakjesLinks.width + dropRow.width
           + haakjesRechts.width + extraMeanWidth

    function shouldDrag(mouseX, mouseY) {
        if (!acceptsDrops)
            return true

        return mouseX <= functionDef.x + functionDef.width
                || (showParentheses
                    && (mouseX <= haakjesLinks.width + haakjesLinks.x
                        || mouseX > haakjesRechts.x))
                || (meanBar.visible && mouseY < meanBar.height + 6)
    }
    ...
    function returnR() {
        ...
        for (var i = 0; i < funcRoot.parameterNames.length; i++)
            compounded += (i > 0 ? ", " : "") + (dropRepeat.itemAt(
                                                     i) === null ? "null" : dropRepeat.itemAt(
                                                                       i).returnR(
                                                                       ))
    ...
    function checkCompletenessFormulas() {
        return dropRepeat.checkCompletenessFormulas()
    }
    function convertToJSON() {
        return dropRepeat.convertToJSON()
    }

    function getParameterDropSpot(param) {
        return dropRepeat.getParameterDropSpot(param)
    }
    ...
    Item {
        ...
        Text {
            ...
            text: funcRoot.isMean || funcRoot.isAbs
                  || funcRoot.isRoot ? "" : functionName
        ...
        Repeater {
            ...
            property var rightMostEmptyDropSpot: function () {
                ...
                for (var i = funcRoot.parameterNames.length - 1; i >= 0; i--) {
                    ...
                    if (dropSpot.containsItem !== null) {
                        ...
                        if (subResult === null)
                            // cant put anything there but maybe we can return the previous (and thus empty dropspot?)
            ...
            function convertToJSON() {
                ...
                for (var i = 0; i < funcRoot.parameterNames.length; i++) {
                    ...
                    var argJson = dropSpot.containsItem
                            === null ? null : dropSpot.containsItem.convertToJSON()
                    jsonObj.arguments.push({
                                               name: funcRoot.parameterNames[i],
                                               dropKeys: funcRoot.parameterDropKeys[i],
                                               argument: argJson
                                           })
                }
                return jsonObj
            ...
            Item {
                ...
                DropSpot {
                    ...
                    droppedShouldBeNested: false // funcRoot.parameterNames.length === 1 && !funcRoot.isAbs && !funcRoot.isMean
