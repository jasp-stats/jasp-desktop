...
DropArea {
    ...
    onEntered: {
        ...
        if ((containsItem !== null && containsItem !== drag.source)
                || !acceptsDrops) {
            drag.accepted = false
            return
        }
        ...
        var ancestry = parent
        while (ancestry !== null) {
            if ((ancestry.objectName == "DragGeneric"
                 && ancestry.dragChild === drag.source)
                    || ancestry == drag.source) {
                drag.accepted = false
                return
            }
    ...
    Item {
        ...
        TextInput {
            ...
            function createNumber(value) {
                setCreatedObjectUp(numberComp.createObject(dragTarget, {
                                                               value: value,
                                                               canBeDragged: true,
                                                               acceptsDrops: true
                                                           }))
            ...
            function createString(string) {
                setCreatedObjectUp(stringComp.createObject(dragTarget, {
                                                               text: text,
                                                               canBeDragged: true,
                                                               acceptsDrops: true
                                                           }))
            ...
            function setCreatedObjectUp(obj) {
                ...
                dragTarget.width = Qt.binding(function () {
                    return obj.width
                })
        ...
        Rectangle {
            id: errorMarker
            z: -2
            visible: (dragTarget.iWasChecked
                      && dragTarget.containsItem === null)
            radius: width
            anchors.fill: parent
            color: "#BB0000"
        }
