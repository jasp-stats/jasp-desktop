...
function SVGGraph(wrapper) {
	...
	this._chartCont = this._wrapper.svg(0, 0, 600, 400, {class_: 'svg-graph'}); // The main container for the graph
	...
	_drawAxis: function(axis, id, x1, y1, x2, y2) {
		...
		if (axis._ticks.major) {
			...
			while (major <= axis._scale.max || minor <= axis._scale.max) {
				...
				if (cur == major) {
                    this._wrapper.text(gt, (horiz ? v : x1 - size), (horiz ? y1 + 2 * size : v + 4),  // + 4 by jonathon !
