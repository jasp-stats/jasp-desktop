...
RowControls::RowControls(ListModel* parent, QVector<QQmlComponent *> &components, const QMap<QString, Option*>& rowOptions)
	: QObject(parent), _parentModel(parent)
{
	...
	for (QQmlComponent* comp : components)
	{
		...
		context->setContextProperty("isRowComponent", true);
		...
		context->setContextProperty("rowIndex",	-1);
		context->setContextProperty("rowValue", "");
