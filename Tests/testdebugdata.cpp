...
void TestDebugData::testEmptyValuesWithoutLabels()
{
	...
	//An empty value of the workspace, as set in the preferences
	contBinom->setHasCustomEmptyValues(false);
	QVERIFY2(missing(asScale(contBinom, {})) == 0,		"contBinom should not have missing values after disabling custom empty values!");

	_data->setWorkspaceEmptyValues({"1"});
	QVERIFY2(missing(asScale (contBinom, {})) == 42,	"The 1s of contBinom should be missing as scale with 1 as workspace empty value!");
	QVERIFY2(missing(asLevels(contBinom, {})) == 42,	"The 1s of contBinom should be missing as nominal with 1 as workspace empty value!");
