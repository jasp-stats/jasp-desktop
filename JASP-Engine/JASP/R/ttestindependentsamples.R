TTestIndependentSamples <- function(dataset = NULL, options, perform = "run",
    ...
    init <- .initialize_ttest(dataset, options, perform)
...
.ttestIndependentSamplesTTest <- function(dataset, options, perform) {
    ...
    ttest <- list()
    wants_effect <- options$effectSize
    wants_difference <- options$meanDifference
    wants_confidence <- options$confidenceInterval
    
    ## can make any combination of the following tests:
    wants_welchs <- options$welchs
    wants_students <- options$students
    wants_wilcox <- options$mannWhitneyU
    ...
    if (!any(wants_welchs, wants_students, wants_wilcox)) {
        wants_students <- TRUE
    }
    
    all_tests <- c(wants_students, wants_welchs, wants_wilcox)
    only_test <- sum(all_tests) == 1
    
    ## get the right title of the table
    if (wants_wilcox && only_test) {
        title <- "Mann-Whitney U Test"
        
        testStat <- "W"
        ## additionally, Wilcoxon's test doesn't have degrees of freedoms
        fields <- fields[-3]
    } else if (wants_welchs && only_test) {
        title <- "Welch's T-Test"
        testStat <- "t"
    } else if (wants_students && only_test) {
    ...
    if (ready) {
        ...
        rowNo <- 1
        which_tests <- list("1" = wants_students, "2" = wants_welchs, "3" = wants_wilcox)
        ...
        for (variable in options$variables) {
            ...
            for (test in seq_len(length(which_tests))) {
                
                current_test <- which_tests[[test]]
                ...
                row <- try(silent = FALSE, expr = {
                    ...
                    if (test == 3) {
                        ...
                        what_test <- ifelse(test == 2, "Welch's", "Student's")
                    ...
                    if (!wants_welchs && wants_students) {
                    ...
                    res <- list(v = variable, test = what_test, df = df, p = p,
                                md = m, d = d, lowerCI = ciLow, upperCI = ciUp,
...
.ttestIndependentSamplesDescriptives <- function(dataset, options, perform,
    ...
    is_allright <- function(variable, options, state = NULL, diff = NULL) {
    ...
    for (variable in variables) {
        ...
        if (is_allright(variable, options, state, diff)) {
            
            state_dat <- state$results$descriptives$data
			descriptivesVariables <- as.character(length(state_dat))
			
			for (i in seq_along(state_dat))
				descriptivesVariables[i] <- state_dat[[i]]$variable
			
			indices <- which(descriptivesVariables == variable)
			data[[length(data) + 1]] <- state_dat[[indices[1]]]
			data[[length(data) + 1]] <- state_dat[[indices[2]]]
        ...
        if (length(levels) != 2) {
            ...
            for (variable in variables) {
                ...
                if (is_allright(variable, options, state, diff)) {
                    ...
                    data[[rowNo]] <- state_dat[[indices[1]]]
                    data[[rowNo]] <- state_dat[[indices[2]]]
