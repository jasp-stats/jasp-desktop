					...
					row <- try(silent = FALSE, expr = {
						...
						if (test == 3) {
							...
							if (wantsConfidenceEffSize){
							  ...
							  dfCohen <- sum(ns) - 2
							  end1 <- stat
							  while( pt(q = stat, df = dfCohen, ncp = end1) > alphaLevels[1]){
							    end1 <- end1 + st
							  }
							  ncp1 <- uniroot(function(x) alphaLevels[1] - pt(q = stat, df = dfCohen, ncp = x),
							                  c(2 * stat - end1, end1))$root
				
							  end2 <- stat
							  while( pt(q = stat, df = dfCohen, ncp = end2) < alphaLevels[2]){
							    end2 <- end2 - st
							  }
							  ncp2 <- uniroot(function(x) alphaLevels[2] - pt(q = stat, df = dfCohen, ncp = x),
