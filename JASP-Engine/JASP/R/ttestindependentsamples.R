            ...
            if (test == 3) {
              ...
              if (wantsEffect) {
                ...
                if (options$effectSizesType == "cohensD") {
                  ...
                  a <- sum(ns) - 2
                  correction_J <- gamma(a / 2) / (sqrt(a / 2) * gamma((a - 1) / 2)) # supposedly; use approx.
                  d <- .clean(as.numeric((ms[1] - ms[2]) / sdPooled)) * correction_J # less biased / corrected version
                }
              ...
              if (wantsConfidenceEffSize){
                ...
                confIntEffSize <- confidenceLimitsEffectSizes(t.value = stat, df = dfEffSize, alpha.lower = alphaLevel, 
                ...
                confIntEffSize <- sort(unlist(confIntEffSize))
              }
            }
...
confidenceLimitsEffectSizes <- function(ncp, df, conf.level=.95, alpha.lower=NULL, alpha.upper=NULL, t.value, tol=1e-9, sup.int.warns=TRUE, ...)
{
