...
SummaryStatsTTestBayesianPairedSamples <- function(dataset=NULL, options, perform = 'run', callback)
{
	...
	{
		return(list(results=results, status="complete", state=list(options=options, results=results, bayesFactorObject=bayesFactorObject, bayesFactorRobustnessPlot=bayesFactorRobustnessPlot, priorAndPosteriorPlot=priorAndPosteriorPlot, rowsTTestBayesianPairedSamples=rowsTTestBayesianPairedSamples)))
	}
}
