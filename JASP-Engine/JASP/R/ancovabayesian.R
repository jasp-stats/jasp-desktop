AncovaBayesian	 <- function(dataset=NULL, options, perform="run", callback=function(...) 0, ...) {
	...
	if (options$dependent != "" && length(options$modelTerms) > 0 ){
		posterior[["title"]] <- paste("Model Comparison - ",options$dependent,sep="")
	...
	if (options$outputEffects == TRUE) {
		...
		if(options$effectsStepwise == FALSE){
			fields <- list(
		...
		if (options$dependent != "" && length(options$modelTerms) > 0) {
			effect[["title"]] <- paste("Analysis of Effects - ",options$dependent,sep="")
...
.effectResultsBayesianAnCova <- function( nmodels = 0, options = NULL, posterior.probability = NULL, n.eff = NULL,
	n.mod.0 = NULL, match.eff.mod = NULL, match.eff.interactions = NULL, withmain = NULL, effects = NULL, models.tab = 	NULL, pprior  = NULL, BFmodels  = NULL, BFmain.0  = NULL, errormain.0 = NULL, model.formula = NULL,
	...
	# 2. Inclusion probabilities and Bayes factors
		...
		if(options$effectsStepwise == TRUE){
	...
	# to select the Full and Reduced model to compare
		...
		for (e in 1:n.eff) {
			...
			#Should then compare against the null
				...
				r[["BF<sub>Backward</sub>"]] <- .clean(BF.Backward)
				r[["% errorB"]] <- .clean(error.Backward)
				r[["BF<sub>Forward</sub>"]] <- BFbottom
				r[["% errorF"]] <- errorbottom
			}
	...
	#Null model and one more
		...
		if(options$effectsStepwise == TRUE){
			r[["BF<sub>Backward</sub>"]] <- ""
			r[["% errorB"]] <- ""
			r[["BF<sub>Forward</sub>"]] <- .clean(BFmain.0[2])
			r[["% errorF"]] <- .clean(errormain.0[2]*100)
		}
		effect.results[[length(effect.results)+1]] <- r
	} else if (nmodels == 0){
		for (term in options$modelTerms) {
