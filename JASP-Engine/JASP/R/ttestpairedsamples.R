TTestPairedSamples <- function(dataset = NULL, options, perform = "run",
    ...
    init <- .initialize_ttest(dataset, options, perform, type = 'paired')
...
.ttestPairedSamples <- function(dataset, options, perform) {
    ...
    wants_confidence <- options$confidenceInterval # pah! can't get that classically :P
    wants_students <- options$students
    wants_wilcox <- options$wilcoxonSignedRank
    
    ## if no checkbox is checked, present a Student's t-test
    if (!wants_students && !wants_wilcox) {
        wants_students <- TRUE
    }
    
    all_tests <- c(wants_students, wants_wilcox)
    ...
    which_tests <- list("1" = wants_students, "2" = wants_wilcox)
    ...
    for (pair in options$pairs) {
        ...
        for (test in seq_len(length(which_tests))) {
            ...
            is_first <- (rowNo %% 2 == 1 && sum(all_tests) == 2) || (sum(all_tests) == 1)
            row[["v1"]] <- ifelse(is_first, p1, "")
            row[["sep"]] <- ifelse(is_first, "-", "")
            row[["v2"]] <- ifelse(is_first, p2, "")
