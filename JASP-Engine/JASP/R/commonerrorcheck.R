...
.quitInternal <- function(message, functionName = NULL) {
	# Function to gracefully signal a user that an unexpected error occured.
	if (is.null(functionName)) {
		s <- sys.calls()
		functionName <- as.character(s[length(s) - 1L])
	}

	e <- sprintf("An internal error occured in %s. %s.", functionName, message)
	stop(e, call. = FALSE)
}
