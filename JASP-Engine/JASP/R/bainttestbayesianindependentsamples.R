...
BainTTestBayesianIndependentSamples <- function(jaspResults, dataset, options, state=NULL) {
	...
	dataset											<- .readDataBainTwoSample(options, dataset)
    # Null state
    if(is.null(state))
      state 										<- list()
    # Pass the title
    jaspResults$title 								<- "Bayesian Informative Independent Samples T-Test"
    # Create the main results table
    .bainTwoSampleTable(dataset, options, jaspResults)
...
.bainTwoSampleTable <- function(dataset, options, jaspResults) {
  ...
  bainTable                      <- createJaspTable("Independent Samples T-test Result")
...
.readDataBainTwoSample <- function(options, dataset){
	...
	if (is.null(dataset)){   
        if (options$missingValues == "excludeListwise") {
            dataset 								<- .readDataSetToEnd(columns.as.numeric=all.variables, columns.as.factor=grouping, exclude.na.listwise=c(all.variables, grouping))
        } else {            
            dataset 								<- .readDataSetToEnd(columns.as.numeric=all.variables, columns.as.factor=grouping, exclude.na.listwise=grouping)
        }
    }
