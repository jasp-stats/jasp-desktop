...
TTestBayesianOneSample <- function(dataset=NULL, options, perform="run", callback=function(...) 0, ...) {
	...
	if (length(options[["variables"]]) > 0)
	{
		...
		### plotPriorAndPosterior function ####
		plotPosterior.ttest <- function(x= NULL, oneSided, iterations= 10000, rscale, lwd= 2, cexPoints= 1.5, cexAxis= 1.2, cexYlab= 1.5, cexXlab= 1.5, cexTextBF= 1.4, cexCI= 1.1, cexLegend= 1.4, lwdAxis= 1.2){
		...
		### plotSequentialAnalysis/plotSequentialAnalysisRobustness function ###
		plotSequentialBF.ttest <- function(x= NULL, y= NULL, rscale= options$priorWidth, oneSided= oneSided, lwd= 2, cexPoints= 1.4, cexAxis= 1.2, cexYlab= 1.5, cexXlab= 1.6, cexTextBF= 1.4, cexText=1.2, cexLegend= 1.4, cexEvidence= 1.6, lwdAxis= 1.2, plotDifferentPriors= FALSE){
			...
			for(i in seq_along(x)){
				if(i == 1){
					...
					BF <- BayesFactor::ttestBF(x = x[1:i], nullInterval= nullInterval, rscale= r)
			...
			for(i in seq_along(x)){
				if(i == 1){
					...
					BF <- BayesFactor::ttestBF(x = x[1:i], nullInterval= nullInterval, rscale= "ultrawide")
			...
			for(i in seq_along(x)){
				if(i == 1){
					...
					BF <- BayesFactor::ttestBF(x = x[1:i], nullInterval= nullInterval, rscale= "medium")
			...
			# display BF10 value
			BFe <- BayesFactor::ttestBF(x=x, y=y, nullInterval= nullInterval, rscale= r)
		...
		### plotBayesFactorRobustness function ###
		plotBF.robustnessCheck.ttest <- function(x= NULL, y= NULL, rscale= options$priorWidth, oneSided, lwd= 2, cexPoints= 1.4, cexAxis= 1.2, cexYXlab= 1.5,  cexText=1.2, cexLegend= 1.4, lwdAxis= 1.2 , cexEvidence= 1.6){ 
			...
			for(i in seq_along(rValues)){
				BF <- BayesFactor::ttestBF(x = x, nullInterval= nullInterval, rscale= rValues[i])
				BF10[i] <- BayesFactor::extractBF(BF, logbf = FALSE, onlybf = F)[1, "bf"]
			}
			
			# BF10 "medium" prior
			BF10m <- BayesFactor::ttestBF(x = x, nullInterval= nullInterval, rscale= "medium")
			BF10m <- BayesFactor::extractBF(BF10m, logbf = FALSE, onlybf = F)[1, "bf"]
			
			# BF10 "wide" prior
			BF10w <- BayesFactor::ttestBF(x = x, nullInterval= nullInterval, rscale= "wide")
			BF10w <- BayesFactor::extractBF(BF10w, logbf = FALSE, onlybf = F)[1, "bf"]
			
			# BF10 "ultrawide" prior
			BF10ultra <- BayesFactor::ttestBF(x = x, nullInterval= nullInterval, rscale= "ultrawide")
			BF10ultra <- BayesFactor::extractBF(BF10ultra, logbf = FALSE, onlybf = F)[1, "bf"]
			
			# BF10 user prior
			BF10user <- BayesFactor::ttestBF(x = x, nullInterval= nullInterval, rscale= r)
