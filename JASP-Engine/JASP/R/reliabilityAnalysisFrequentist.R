...
.frequentistReliabilityDerivedOptions <- function(options) {
  ...
  derivedOptions <- list(
    ...
    namesEstimators     = list(
      tables = c(gettextf("McDonald's %s","\u03C9"), gettextf("Cronbach's %s","\u03B1"), gettextf("Guttman's %s","\u03BB2"), gettextf("Guttman's %s","\u03BB6"),
                 gettext("Greatest Lower Bound"), gettext("Average interitem correlation"), gettext("mean"), gettext("sd")),
      tables_item = c(gettextf("McDonald's %s","\u03C9"), gettextf("Cronbach's %s","\u03B1"), gettextf("Guttman's %s","\u03BB2"), gettextf("Guttman's %s","\u03BB6"),
                      gettext("Greatest Lower Bound"), gettext("Item-rest correlation"), gettext("mean"), gettext("sd")),
      coefficients = c(gettextf("McDonald's %s","\u03C9"), gettextf("Cronbach's %s","\u03B1"), gettextf("Guttman's %s","\u03BB2"), gettextf("Guttman's %s","\u03BB6"),
...
.frequentistReliabilityItemTable <- function(jaspResults, model, options) {
  ...
  if (!is.null(relyFit)) {
    if (options[["mcDonaldScale"]] && !is.null(relyFit[["freq"]][["omega.item.error"]])) {
      itemTable$addFootnote(gettextf("McDonald's %s estimation method for item-dropped statistics
                                     switched to PFA because the CFA did not find a solution.","/u03C9"))
    }
  ...
  if (!is.null(relyFit)) {
    if (is.null(opts)) {
      ...
      if (!is.null(relyFit[["freq"]][["omega.error"]])) {
          fitTable$addFootnote(gettextf("Fit measures cannot be displayed because the McDonald's %s
                                       estimation method switched to PFA as the CFA did not find a solution.","\u03C9"))
      }
    } else {
