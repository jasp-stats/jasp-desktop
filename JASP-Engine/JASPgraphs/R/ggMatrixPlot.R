...
makeLabels.default <- function(label, angle = 0, size = 1, family = graphOptions("family")) {
    ...
    scx <- ggplot2::scale_x_continuous(limits = c(-1, 1))
    scy <- ggplot2::scale_y_continuous(limits = c(-1, 1))
    if (!is.null(family)) {
        return(
            ggplot2::ggplot(df, aes(x, y, label = label)) +
                ggplot2::geom_text(angle = angle, size = size, family = family, parse = parse, vjust = "center") +
                scx + scy +
                ggplot2::theme_void() +
        ...
        return(
            ggplot2::ggplot(df, aes(x, y, label = label)) +
                ggplot2::geom_text(angle = angle, size = size, parse = parse, vjust = "center", hjust = "center") +
                scx + scy +
                ggplot2::theme_void() +
                ggplot2::theme(plot.background = ggplot2::element_rect(fill = "transparent"))
        )
